...
class Interpreter:
    ...
    """
    def __init__(self, ast : AbstractSyntaxTree, storage : Storage):
        self.ast = ast
    ...
    def walk_FunctionCall(self, node):
        if node.function_name == "demande":
            #TODO eventually add support for other input means than console.
            user_input = input(f"{self.walk(node.args[0])} ")
            return float(user_input) if re.fullmatch(r"^-?(([0-9]+\.?[0-9]+)|([0-9]+))$", user_input) else user_input
        elif node.function_name == "affiche":
            #TODO eventually add support for other output means than console.
            for arg in node.args:
                print(self.walk(arg), end=" ")
            print()
        else:
